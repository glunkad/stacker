...
def main():
    ...
    # while not rospy.is_shutdown():
    while found_count < 2:
        local_vel_pub_edrone_1.publish(vel)
        for setpoint in setpoints_0:
            ...
            while start_search:
                ...
                if aruco_check != 0: 
                    x1 = pos1.pose.position.x = stateMt.e_drone_1_pos1.pose.position.x+1
                    y1,z1 = stateMt.e_drone_1_pos1.pose.position.y+1, stateMt.e_drone_1_pos1.pose.position.z
                    while not found:
                        try:    
                            cx,cy=aruco.Calculate_orientation(aruco.detect_ArUco(stateMt.e_drone_1_img))
                            if(abs(cx - 200 )<= 5 and  abs(cy - 200 )<= 3 ) :
                                print("Ready to pick up ")
                                z1 = pos1.pose.position.z = -0.1
                        ...
                        while stateMt.e_drone_1_grip.data != "True" and found:
                            found = 0
                            rate.sleep()
                        if stateMt.e_drone_1_pos1.pose.position.z < 0.5:
                            found_count += 1
