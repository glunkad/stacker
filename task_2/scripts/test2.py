...
class Mavros_Test(unittest.TestCase):
	def __init__(self, *args):
		super(Mavros_Test, self).__init__(*args)
	...
	def setUp(self):
		...
		self.global_position_sub = rospy.Subscriber('mavros/global_position', NavSatFix, self.global_position_callback)
	...
	def wait_for_topics(self,timeout):
		...
		self.assertTrue(simulation_ready, (
			"failed to hear from all subscribed simulation topics | topic ready flags: {0} | timeout(seconds): {1}".format(self.sub_topics_ready, timeout)
			))
	...
	def wait_for_landed_state(self, desired_landed_state, timeout, index):
		...
		self.assertTrue(landed_state_confirmed, (
			"landed state not detected | desired: {0}, current: {1} | index: {2}, timeout(seconds): {3}".format(
				...
				mavutil.mavlink.enums['MAV_LANDED_STATE'][self.extended_state.landed_state].name,index,timeout)
			))
	...
	def wait_for_vtol_state(self, transition, timeout, index):
		...
		self.assertTrue(transitioned,(
			"transitioned not detected | desired: {0}, current: {1} | index: {2} timeout(seconds): {3}".format(
		...
		for i in xrange(timeout * loop_freq):
			try:
				...
				if res.success:
					...
					rospy.loginfo(
						"MAV_TYPE received | type: {0} | seconds: {1} of {2}".format(mavutil.mavlink.enums['MAV_TYPE'][self.mav_type].name, i / loop_freq, timeout)
						)
					break
			except rospy.ServiceException as e:
				rospy.logerr(e)
...
class Offboard_Control(Mavros_Test):

	def setUp(self):
		super(Offboard_Control, self).setUp()
	...
	def tearDown(self):
		super(Offboard_Control, self).tearDown()
	...
	def test_posctl(self):
		...
		self.set_mode("OFFBOARD", 20)
		self.set_arm(True, 20)
		rospy.loginfo("run mission")
		positions = ((0, 0, 0), (0, 0, 10), (10, 0, 10), (10, 10, 10),(0, 10, 10), (0, 0, 10))
		for i in xrange(len(positions)):
			self.reach_position(positions[i][0],positions[i][1], positions[i][2], 100)
...
if __name__ == "__main__":
	...
	rostest.rosrun(PKG, 'test2',Offboard_Control)
