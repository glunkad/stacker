...
class e_drone(pick_n_place,Aruco,stateMoniter):
    ...
    def __init__(self,drone_no,drop):
        ...
        self.off, self.alt = 0.5,3.00
        self.found_count = 0
        self.debug ,self.found =1, 0
                ...
                while not self.found:
                    try:    
                        ...
                        if(abs(cx - 200 )<= 3 and  abs(cy - 200 ) <= 3 ) :
                            ...
                            x1= self.epos.pose.position.x  = self.pos.pose.position.x + 0.8
                            self.local_pos_pub.publish(self.epos)
                            print("increase ++ 0.5")
                        elif(cx - 200 > 3 and cx-200 < 2) :
                            x1= self.epos.pose.position.x  = self.pos.pose.position.x + 0.3
                            self.local_pos_pub.publish(self.epos)
                            print("increase ++ 0.3")
                        if(cy - 200 < -3.1) :
